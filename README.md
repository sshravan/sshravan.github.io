...
### Deploy to github.io
```bash
git clone git@github.com:sshravan/sshravan.github.io.git -b dev ../sshravan.github.io
bundle exec jekyll build
cp -r _site/* ../sshravan.github.io
cd ../sshravan.github.io
git add .
git commit -m
git push
```

### Deploy to CS (No longer used.)
```bash
bundle exec jekyll build
# Be sure that ```baseurl``` is set in _config.yml
ssh sshravan@junkfood.cs.umd.edu 'bash -c "cd /fs/wwwusers/sshravan/; rm -rf ./*"'
scp -r _site/* sshravan@junkfood.cs.umd.edu:/fs/wwwusers/sshravan/
ssh sshravan@junkfood.cs.umd.edu 'bash -c "cd /fs/wwwusers/sshravan/; rm README.md LICENSE Gemfile Gemfile.lock; chmod -R 755 .; chmod 644 index.html"'
```
...
http://localhost:4000/~sshravan/
# NOT http://127.0.0.1:4000/~sshravan/
```

### Merging Upstream Changes
```bash
git clone --recursive git@github.com:sshravan/personal-webpage.git sshravan-cs-webpage
git remote add upstream https://github.com/alshedivat/al-folio.git
git fetch upstream
git checkout master
git merge/rebase upstream/master
# git checkout shravan
# git rebase master # Will cause the origin and local to diverge
# git push -f origin shravan # Need to look for other options. As this step is not recommended.
```

### Reduce image resolution
```bash
convert  -resize 5% raw/img/shravan.jpg assets/img/shravan-low-res.jpg
```
